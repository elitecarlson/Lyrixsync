...
Upload an audio file with the song name and artist in the filename. Lyrixsync will hit the iTunes API to fetch metadata like the track name, artist, and album cover, and will also attempt to automatically fetch lyrics from lrclib.net.

From there you can click **sync** to sync the lyrics to the audio. The backend runs the audio through **Demucs** to separate it into vocal and instrumental stems, then uses **stable-ts** for word-level forced alignment, so each word is automatically synced exactly when it's sung.
